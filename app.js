(function() {
	...
	function playButton() {
		...
		if (currentButton === GREEN) {
			SFX_GREEN.play();
			document.getElementById('green').style.backgroundImage = `url(${BTN_GREEN_1})`;
		}
		...
		if (currentButton === YELLOW) {
			SFX_YELLOW.play();
			document.getElementById('yellow').style.backgroundImage = `url(${BTN_YELLOW_1})`;
		}
		if (currentButton === BLUE) {
			SFX_BLUE.play();
			document.getElementById('blue').style.backgroundImage = `url(${BTN_BLUE_1})`;
		}
	...
	function resetButton() {
		if (currentButton === GREEN) {
			document.getElementById('green').style.backgroundImage = `url(${BTN_GREEN_0})`;
		}
		if (currentButton === RED) {
			document.getElementById('red').style.backgroundImage = `url(${BTN_RED_0})`;
		}
		if (currentButton === YELLOW) {
			document.getElementById('yellow').style.backgroundImage = `url(${BTN_YELLOW_0})`;
		}
		if (currentButton === BLUE) {
			document.getElementById('blue').style.backgroundImage = `url(${BTN_BLUE_0})`;
		}
		buttonReset = true;
	}
	...
	$('#start').mousedown(function() {
		SFX_CLICK.play();
		document.getElementById('start').style.backgroundImage = `url(${BTN_START_1})`;
		startGame();
	});
	$('#start').mouseup(function() {
		document.getElementById('start').style.backgroundImage = `url(${BTN_START_0})`;
	});
	$('#strict').mousedown(function() {
		SFX_CLICK.play();
		document.getElementById('strict').style.backgroundImage = `url(${BTN_STRICT_1})`;
	});
	$('#strict').mouseup(function() {
		if (strictOn) {
			document.getElementById('strict').style.backgroundImage = `url(${BTN_STRICT_0})`;
			strictOn = false;
		} else {
			document.getElementById('strict').style.backgroundImage = `url(${BTN_STRICT_1})`;
			strictOn = true;
